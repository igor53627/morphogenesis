...
mod admin_snapshot {
    ...
    #[tokio::test]
    async fn snapshot_endpoint_rotates_epoch_and_updates_metadata() {
        let state = test_state();
        let path = unique_temp_path("network_api_snapshot.bin");
        fs::write(&path, vec![0xCDu8; state.row_size_bytes * 4]).expect("write snapshot fixture");

        let body = serde_json::json!({
            "r2_url": path.to_string_lossy(),
            "seeds": [11, 22, 33],
            "block_number": 77,
            "state_root": format!("0x{}", "aa".repeat(32)),
        })
        .to_string();
        ...
        assert_eq!(response.status(), StatusCode::OK);
        assert_eq!(state.global.load().epoch_id, 43);
        assert!(
            state
                .global
                .load_pending()
            ...
            "snapshot rotation should clear pending"
        );

        let epoch_response = create_router(state)
            .oneshot(
                Request::builder()
                    .uri("/epoch")
                    .body(Body::empty())
            ...
            .await
            .unwrap();
        let epoch_body = axum::body::to_bytes(epoch_response.into_body(), 4096)
            .await
            .unwrap();
        let json: serde_json::Value = serde_json::from_slice(&epoch_body).unwrap();

        assert_eq!(json["epoch_id"], 43);
        assert_eq!(json["seeds"], serde_json::json!([11, 22, 33]));
        assert_eq!(json["block_number"], 77);
    }

    #[tokio::test]
    async fn snapshot_endpoint_rejects_missing_local_file() {
        let state = test_state();
        let missing = unique_temp_path("network_api_missing_snapshot.bin");
        let body = serde_json::json!({
            "r2_url": missing.to_string_lossy(),
        })
        .to_string();
...
mod gpu_query {
    ...
    fn test_state_for_gpu() -> Arc<AppState> {
        ...
        Arc::new(AppState {
            ...
            state_root: [0xAB; 32],
            epoch_rx: rx,
            page_config: None, // GPU query doesn't currently check page_config, but maybe it should
            #[cfg(feature = "cuda")]
            gpu_scanner: None, // We don't have a real GPU scanner in this test env
            #[cfg(feature = "cuda")]
            gpu_matrix: None,
        })
    }

    fn test_chacha_keys(target: usize) -> String {
        let params = ChaChaParams::new(8).unwrap();
        let (k0, _k1) = generate_chacha_dpf_keys(&params, target).unwrap();
        // Use same key for all 3 slots for simplicity
        let hex0 = format!("0x{}", hex::encode(k0.to_bytes()));
    ...
    async fn gpu_query_rejects_wrong_key_count() {
        ...
        let hex0 = format!("0x{}", hex::encode(k0.to_bytes()));
        let body = format!(r#"{{"keys":["{hex0}","{hex0}"]}}"#);
