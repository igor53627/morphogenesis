...
2026-02-24: Introduced a canonical production ops bundle under ops/: multi-stage Dockerfiles for morphogen-server and morphogen-rpc-adapter, plus a compose orchestration manifest with server A/B replicas, adapter, and supporting code-data service with health checks/startup dependencies (depends_on with service_healthy). Included env templates for server A/B and adapter in ops/env/, then documented required env vars, secret handling, startup ordering, and health verification in ops/README.md. Added runbooks for deploy/rollback/incident response/maintenance and the artifact validation script scripts/test_ops_artifacts.sh; followed red->green flow (initially failed on missing ops/README.md, then passed after implementation). Validated compose manifest rendering via docker compose config with required env vars set.
