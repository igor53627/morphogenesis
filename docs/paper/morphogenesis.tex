...
\title{Morphogenesis PIR:\\2-Server DPF-PIR at Memory Bandwidth\\{$O(N)$ Queries, $O(1)$ Updates}}
\author{}
\date{}
...
\textbf{Repeated Queries.} The server cannot link repeated queries for the same account. Each query consists of DPF keys that are computationally indistinguishable from random; the server never learns the target positions $\{h_1, h_2, h_3\}$. Additionally, chain updates modify account data between epochs, so even payload sizes reveal nothing about query targets.
...
\begin{itemize}
  \item \textbf{Hardware:} AMD EPYC 9375F (32 cores, 3.8 GHz base), 512 GB DDR5-4800 (8 channels).
  \item \textbf{Theoretical peak:} $8 \times 38.4 = 307$ GB/s (DDR5-4800 per channel).
  \item \textbf{Software:} Rust 1.75, AVX-512 + VAES intrinsics, rayon for parallelism.
  \item \textbf{Dataset:} Synthetic random rows; real Ethereum state fixture (102k accounts from Sepolia).
\end{itemize}

\textbf{Methodology.} Each benchmark: 10 warmup iterations (page faults), 100 timed iterations, report median. Throughput = $N \times \ell / \text{time}$.

\subsection{Memory Bandwidth Results}

\begin{table}[h]
\centering
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Configuration} & \textbf{Threads} & \textbf{Throughput} & \textbf{Notes} \\
\midrule
Single-threaded, 8-row unroll & 1 & 28.5 GB/s & Baseline \\
Parallel (rayon), 8-row unroll & 32 & 393 GB/s & 13.8$\times$ scaling \\
\bottomrule
\end{tabular}
\caption{Scan throughput on EPYC 9375F. 393 GB/s exceeds theoretical DDR5 peak due to cache effects on synthetic data.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Projected query latency. (TBD: end-to-end benchmarks with network + delta scans.)}
\end{table}

\textbf{Reproducibility.} Benchmark code available at \texttt{crates/morphogen-server/benches/}. Run: \texttt{cargo bench --features network}.
...
In biology, a \textbf{morphogen} is a signaling molecule that diffuses through tissue. Each cell samples the local concentration: high concentration triggers differentiation, low concentration keeps the cell dormant. In our protocol, the \textbf{DPF acts as the morphogen}. The server evaluates it at every row index. At $N-1$ positions the DPF outputs 0 (dormant); at exactly one position it outputs 1 (differentiated). Only that row contributes to the response, extracting structured data from an otherwise uniform scan.
